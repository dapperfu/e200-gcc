...
md_assemble_via_translation(char * str)
{
  ...
  if ((opcode->flags & PPC_OPCODE_VLE) != PPC_OPCODE_VLE && (ppc_cpu & PPC_OPCODE_VLE) == PPC_OPCODE_VLE)
    {
    /* Try to translate */
    if (opcode->translate != NULL)
      {
  ...
  else if ((opcode->flags & PPC_OPCODE_VLE) != PPC_OPCODE_VLE && (ppc_cpu & PPC_OPCODE_VLE) != PPC_OPCODE_VLE)
    {
      as_bad(_("the use of -ppc_asm_to_vle requires -mvle."));
    }
  else
    {
      if (opcode->translate != NULL)
  ...
  if (try_to_translate)
    {
      ...
      i = (*opcode->translate)(insn, opcode, (const char **)literal_operands, new_str, &errmsg);
...
md_assemble_really(char * str)
{
  ...
  if ((opcode->flags & PPC_OPCODE_VLE) != PPC_OPCODE_VLE && (ppc_cpu & PPC_OPCODE_VLE) == PPC_OPCODE_VLE)
    {
      /* LSP and SPE2 instructions are valid for VLE mode */
      if ( (opcode->flags & (PPC_OPCODE_LSP | PPC_OPCODE_SPE2)) == 0)
        {
        as_bad(_("Illegal instruction for VLE mode: `%s'"), str);
        return;
        }
    }
...
ppc_section_flags (flagword flags, bfd_vma attr, int type)
{
	  ...
	  //[Af] Shouldn't there be (attr & SHF_PPC_VLE) instead (attr == SHF_PPC_VLE) ?
	  if (attr == SHF_PPC_VLE)
	    flags |= SEC_PPC_VLE;
...
bfd_vma
ppc_elf_section_letter (int letter, char **ptrmsg)
{
  if (letter == 'v')
    return SHF_PPC_VLE;
